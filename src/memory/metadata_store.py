...
class MetadataStore:
    ...
    async def _commit_with_retry(self, max_retries: int = 5) -> None:
        """commit をランダム待機リトライ付きで実行する。

        SQLite は同一ファイルへの同時書き込みで "database is locked" を返す場合がある。
        デッドロック（循環依存）は構造上発生しないため、ランダム待機後にリトライする。
        """
