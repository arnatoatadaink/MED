...
class MemoryManager:
    ...
    async def initialize(self) -> None:
        ...
        FAISS インデックスをディスクから復元する。ファイルが存在しない場合
        （初回起動・新規ドメイン）は警告なしでスキップする。
        """
        await self.store.initialize()
        if self.teacher_registry is not None:
            await self.teacher_registry.initialize()

        # FAISS インデックスをディスクから復元（再起動時のデータ保持）
        try:
            await asyncio.get_event_loop().run_in_executor(None, self.faiss.load)
            logger.info(
                "FAISS indices loaded from disk: %s",
                self.faiss.domain_stats(),
            )
        except FileNotFoundError:
            logger.info("FAISS index files not found — starting with empty indices (first startup)")
        except Exception:
            logger.warning("Failed to load FAISS indices from disk — starting with empty indices", exc_info=True)

        self._initialized = True
        logger.info("MemoryManager initialized")

    async def close(self) -> None:
        """リソースを解放する。シャットダウン前に FAISS インデックスをディスクに保存する。"""
        # FAISS を保存してからリソースを解放する（データ損失防止）
        try:
            await asyncio.get_event_loop().run_in_executor(None, self.faiss.save)
            logger.info("FAISS indices saved on close: %s", self.faiss.domain_stats())
        except Exception:
            logger.exception("Failed to save FAISS indices on close")
    ...
    async def add(self, doc: Document) -> str:
        ...
        doc_id = await self._add_internal(doc)

        # FAISS をディスクに保存（再起動後もデータを維持するため追加のたびに保存）
        # run_in_executor で I/O をブロックせずに実行する
        try:
            await asyncio.get_event_loop().run_in_executor(None, self.faiss.save)
        except Exception:
            logger.warning("FAISS save failed after add doc=%s (non-fatal)", doc.id[:8])

        return doc_id

    async def _add_internal(self, doc: Document) -> str:
        """FAISS 保存を除く追加処理（add_batch で再利用）。"""
        # 埋め込みがなければ生成
    ...
    async def add_batch(self, docs: list[Document]) -> list[str]:
        ...
        # バッチ完了後に1回だけ FAISS を保存（追加のたびに保存するより効率的）
        if added:
            try:
                await asyncio.get_event_loop().run_in_executor(None, self.faiss.save)
                logger.info("FAISS indices saved after batch add (%d docs)", len(added))
            except Exception:
                logger.warning("FAISS save failed after batch add (non-fatal)")
    ...
    async def search_hybrid(
        ...
        # ── 4. マージ（FAISS 優先、FTS5/エイリアスを末尾に追加） ──
        combined = faiss_results + extra_results
        return combined[:k]
